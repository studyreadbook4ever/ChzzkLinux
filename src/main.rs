...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // 공식 채널명을 담습니다. 초기값은 입력한 별명
    let mut official_name = alias.to_string(); 

    // Dual Stack 
    if let Some(id) = channels.get(alias) {
        channel_id = id.clone();
        
        // '공식 채널명' 획득!
        let info_url = format!("https://api.chzzk.naver.com/service/v1/channels/{}", channel_id);
        if let Ok(info_resp) = client.get(&info_url)
            .header("User-Agent", "Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/121.0.0.0 Safari/537.36")
            .send().await 
        {
            if let Ok(channel_data) = info_resp.json::<ChannelResponse>().await {
                if let Some(info) = channel_data.content {
                    official_name = info.channelName; // 성공 시 공식명으로 덮어쓰기!
                }
            }
        }

        println!("✅ DB match success: found {} channel on channels.toml!", official_name);

    } else {
        println!("DB match failed: {} search on chzzk for find channel.....", alias);
        
        let search_url = "https://api.chzzk.naver.com/service/v1/search/channels";
        let search_resp = client.get(search_url)
            .query(&[("keyword", alias)])
            .header("User-Agent", "Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/121.0.0.0 Safari/537.36")
            .send().await?.json::<SearchResponse>().await?;

        let mut found = None;
        if let Some(content) = search_resp.content {
            if let Some(first_match) = content.data.first() {
                found = Some((first_match.channel.channelId.clone(), first_match.channel.channelName.clone()));
            }
        }
        ...
        match found {
            Some((id, name)) => {
                channel_id = id.clone();
                official_name = name.clone(); // 검색으로 찾았을 땐 이미 네이버가 이름을 같이 줬으므로 통신 낭비 없이 변수에 저장!
                
                println!("search success! channel name: [{}] (Hash Code: {})", official_name, channel_id);
                
                if let Ok(mut file) = OpenOptions::new().create(true).append(true).open("channels.toml") {
                    if let Err(e) = writeln!(file, "\"{}\" = \"{}\"", alias, channel_id) {
                        eprintln!("channels.toml auto save failure: {}", e);
                    } else {
                        println!("'{}' nickname saved on channels.toml!", alias);
                    }
                }
            }
            None => {
                eprintln!("there are no channel {}....", alias);
                return Ok(());
            }
        }
    }  

    println!("{} channel connecting...", official_name);
